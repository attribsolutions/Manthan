...
import { Scheme_Type__Delete_API, Scheme_Type_API, Scheme_Type_Edit_API, Scheme_Type_Post_API, Scheme_Type_Update_API } from "../../../helpers/backend_helper";
...
function* Get_Scheme_Type_List_GenrFun({ config }) {                            // getList API
    try {




        const response = yield call(Scheme_Type_API, config);
        ...
        yield put(getSchemeTypelistSuccess(newList));
    } catch (error) { yield put(SchemeTypeErrorAction()) }
}
...
function* Delete_Scheme_Type_ID_GenFun({ config }) {                     // delete API
    try {
