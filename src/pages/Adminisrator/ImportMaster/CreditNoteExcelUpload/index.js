...
import {
    ...
    goButtonPartyItemAddPage,
    goButtonPartyItemAddPageSuccess,
} from "../../../../store/actions";
import Dropzone from "react-dropzone"
import {
    InvoiceUploadCalculation,
    downloadDummyFormatHandler,
    fileDetails,
    filterArraysInEntries,
    ExcelReadFile,
    FilterNoOfInvoice
} from "./excelReadFile";
import {
...
import { pageId, url } from "../../../../routes";
import { GoButton_ImportExcelPartyMap_Success } from "../../../../store/Administrator/ImportExcelPartyMapRedux/action";

const CreditNoteExcelUpload = (props) => {
    ...
    const [noOfInvoice, setNoOfInvoice] = useState([]);
    ...
    useEffect(async () => {
        ...
        return () => {
            ...
            dispatch(CredietDebitTypeSuccess([]));
        }
    ...
    useEffect(() => {
        ...
        dispatch(commonPageFieldSuccess(null));
        dispatch(commonPageField(pageId.CREDIT_NOTE_UPLOAD))
    }, []);
    ...
    useEffect(() => {
        ...
        if (isIgnoreNegativeValue && isIgnoreItem && isIgnoreParty) {
            ...
            updateReadJsonDetails.invoice = filterArraysInEntries(readJsonDetail.invoice, conditionFunction);
        ...
        setUpdateReadJsonDetail(updateReadJsonDetails)
    ...
    useEffect(() => {
        ...
        const filteredInvoices = FilterNoOfInvoice(readJsonDetail.invoice, conditionFunction, isIgnoreNegativeValue);
        setNoOfInvoice(filteredInvoices); // Update state with the number of filtered invoices
    ...
    useEffect(() => {
        ...
        const filteredInvoices = FilterNoOfInvoice(readJsonDetail.invoice, conditionFunction, isIgnoreParty);
        setNoOfInvoice(filteredInvoices); // Update state with the number of filtered invoices
    ...
    useEffect(() => {
        ...
        const filteredInvoices = FilterNoOfInvoice(readJsonDetail.invoice, conditionFunction, isIgnoreItem);
        setNoOfInvoice(filteredInvoices); // Update state with the number of filtered invoices

    }, [readJsonDetail, isIgnoreItem]);
    ...
    function goButtonHandler() {
        ...
        const jsonBody = JSON.stringify({
            ...
            IsFieldType: 3// type  3 for Credit Note Fields
        })
    ...
    async function veifyExcelBtn_Handler() {
        ...
        if (extension === ".xlsx") {
            const readjson = await ExcelReadFile({ file: files[0], compareParameter, ItemList })
                ...
                try {
                    ...
                    MapItemList.forEach(function (i) {
                        ...
                        Matching_ItemCode_Objects.forEach(matchingObject => {
                            ...
                            matchingObject.Item_ID = Item_ID
                        });

                        if (Matching_ItemCode_Objects) {
                            Matching_ItemCode_Objects.GST = GST;
                            Matching_ItemCode_Objects.Item_ID = Item_ID
                        }
                    });
            ...
            updatereadJsonDetail.invoice.forEach(async (inv) => {
                ...
                inv.forEach(async (ele) => {
                    ...
                    const calculate = InvoiceUploadCalculation({
                        ...
                        GST: ele.GST,
                        Discount: ele[parArr.Discount],
                        DiscountType: ele[parArr.DiscountType]
                    });
                    invoiceTotalAmount = invoiceTotalAmount + calculate.Amount;

                    const getCustomerFromId = (customerId) => {

                        const customer = PartyMapData.find(c => Number(c.MapCustomer) === customerId);
                        return customer ? customer.Customer : ''; // Return customer ID if found, otherwise empty string
                    };

                    parentObj = {

                        "ImportFromExcel": 1,
                        "CRDRNoteDate": ele[parArr.CRNDate] ? ele[parArr.CRNDate] : '',
                        "Customer": getCustomerFromId(ele[parArr.Customer]) ? getCustomerFromId(ele[parArr.Customer]) : '',
                        ...
                        "Narration": "",
                        "FullNoteNumber": ele[parArr.CreditNoteNo] ? ele[parArr.CreditNoteNo] : '',
                ...
                outerArr.push({ ...parentObj, CRDRNoteItems: CRDRNoteItems })
            });
            const jsonBody = JSON.stringify({ "BulkData": outerArr })

            dispatch(InvoiceExcelUpload_save_action({ jsonBody, btnId, subPageMode: url.CREDIT_NOTE_UPLOAD }));
    ...
    if (!(userPageAccessState === '')) {
        return (
            <React.Fragment>
                ...
                <PageLoadingSpinner isLoading={(compareParamLoading)} />
                        ...
                        <div className="dropzone-previews mt-3" id="file-previews">
                            ...
                            <div id="filedetail">
                                {preViewDivShow ? <Card >
                                    ...
                                    <details>
                                        <summary>&nbsp; &nbsp;  No. of Credit Note: {updatereadJsonDetail.invoice.size}
                                        ...
                                        <div className="error-msg">
                                            <p>
                                                {
                                                    noOfInvoice.map((i) => (<Label>{i} ,&#160;</Label>))
                                                }
                                            </p>
                                        </div>
                                    </details>
                                ...
                                {negativeFigureVerify.Not_Verify_Negative_Figure !== undefined ?
                                    ...
                                    <details>
                                        {negativeFigureVerify.Not_Verify_Negative_Figure === false ? null :
                                            <Row className="mt-2 error-msg"
                                                style={{ margin: "unset", backgroundColor: "#c1cfed" }}>
                                                <Col sm={3} style={{ fontWeight: "bold", fontSize: "15px", paddingLeft: "unset" }}
                                                    className="col-xl-auto">&nbsp;&nbsp;Ignore Item Contain Negative or zero Value</Col>
                                                <Col   >
                                                    <div className="form-check form-switch form-switch-md " style={{ marginTop: "-3px" }}>
                                                        <Input type="checkbox" className="form-check-input"
...
export default CreditNoteExcelUpload
