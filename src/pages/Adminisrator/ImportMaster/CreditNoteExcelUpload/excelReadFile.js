...
export const ExcelReadFile = async ({ file, compareParameter, ItemList = [] }) => {
    ...
    try {
        ...
        if (comparefilter.length === 0) {
            invalidMsg.push(`Credit Note Excel Field Not Map`)
        }
...
export async function fileDetails({ compareParameter = [], Not_Ignore_Item_Array = [], }) {
    ...
    Not_Ignore_Item_Array.forEach((index) => {
        ...
        var invoiceFound = invoiceNO.find(i => (i === (index[fileFiled.CreditNoteNo])))
        ...
        var dateFound = partyNO.find(i => (i === (index[fileFiled.CRNDate])))
        amount = Number(amount) + Number(index[fileFiled.GrandTotal]);

        if ((invoiceFound === undefined)) {
            invoiceNO.push((index[fileFiled.CreditNoteNo]))
        };
        ...
        if ((dateFound === undefined)) {
            invoiceDate.push(date_dmy_func((index[fileFiled.CRNDate])))
        };
    })

    const invoice = await groupBy(Not_Ignore_Item_Array, (index) => {
        return (index[fileFiled.CreditNoteNo])
    })
...
export const FilterNoOfInvoice = (map, conditionFn, isIgnoreNegativeValue = false) => {

    const filteredKeys = Array.from(map.entries()).reduce((acc, [key, value]) => {

        // Check if isIgnoreNegativeValue is true or conditionFn is satisfied
        if (!(isIgnoreNegativeValue) || value.some(conditionFn)) {
            acc.push(key); // Add the key to the accumulator
        }
        return acc;
    }, []);
