...
import { Uploaded_EInvoiceAction } from "../../../store/Sales/Invoice/action";
...
import {
  itemAmounWithGst,
  settingBaseRoundOffOrderAmountFunc,
} from "./util/calculationFunc";
import {
  saveBulkInvoiceAction,
  saveBulkInvoiceActionSuccess,
} from "../../../store/Sales/bulkInvoice/action";
...
const Bulk_Invoice2 = (props) => {
  ...
  const {
    ...
    commonPartyDropSelect:
      state.CommonPartyDropdownReducer.commonPartyDropSelect,
  }));
  ...
  useEffect(async () => {
    if (postMsg.Status === true && postMsg.StatusCode === 200) {
      ...
      try {
        ...
        if (
          systemSetting.AutoEInvoice === "1" &&
          systemSetting.EInvoiceApplicable === "1"
        ) {
          for (const tranasactionId of postMsg.TransactionID) {
            //multiple  transcation ids (comma separate)
            const config = {
  ...
  const saveHandleCallBack = useCallback((bulkData) => {
    ...
    for (const orderInfo of newBulkData) {
      ...
      for (const itemInfo of orderInfo.OrderItemDetails) {
        ...
        if (itemInfo.lessStock) {
          //** */ if Short Short validation check  */
          validMsg.push({
            [itemInfo.ItemName]: `Short Short Quantity ${itemInfo.lessStock} ${
              itemInfo.UnitName?.split(" ")[0]
            }`,
          });
        }
        ...
        for (const stockInfo of itemInfo.StockDetails) {
          ...
          // };
                    ...
                    // Check if the current stock's MRP differs from the first one recorded and if stock is being distributed
                    if (isSameMRPinStock !== '' && isSameMRPinStock !== parseFloat(stockInfo.MRP) && stockInfo.distribute !== 0) {
                        validMsg.push({ [`Order Number ${orderNumber}  (${itemInfo.ItemName})`]: "Multiple MRP’S Invoice not allowed." });
                        break; // Stop further processing for this item since it has multiple MRPs
                    }

                    // Set the MRP for the first time or continue if it's the same as the first one
                    if (isSameMRPinStock === '' || isSameMRPinStock === parseFloat(stockInfo.MRP)) {
                        isSameMRPinStock = parseFloat(stockInfo.MRP);
                    }

                    // Rate validation check
                    if (!Number(stockInfo.Rate) > 0) {
                        validMsg.push({ [itemInfo.ItemName]: "Rate not available." });
                    }
          ...
          if (stockInfo.distribute > 0) {
            ...
            const orderInvoiceItems = {
              ...
              PreviousInvoiceBaseUnitQuantity: Number(
                stockInfo.BaseUnitQuantity
              ).toFixed(3),
      ...
      const orderCalculate = settingBaseRoundOffOrderAmountFunc({
        IsTCSParty,
        IsCustomerPAN,
        sumOfItemAmount,
      });
      ...
      orderIncoiceItems.length > 0 &&
        bulkInvoiceJsonBody.push(orderInvoiceJsonBody);
    }
