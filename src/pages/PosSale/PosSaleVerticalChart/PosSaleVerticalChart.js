...
export default function PosSaleVerticalChart() {

    const axios = require('axios');
    const dispatch = useDispatch();
    const current = new Date();
    const month = current.getMonth() + 1;
    const currentDate = `${current.getFullYear()}-${month < 10 ? `0${month}` : `${month}`}-${current.getDate()}`;
    ...
    function AxiosApiCallFunction(FromDate,Todate) {
        ...
        var config = {
            method: 'get',
            url: `http://web.chitalebandhu.in:8080/ChartjsAPI/api/SaleChart/GetChartHoriData?FromDate=${FromDate}&ToDate=${Todate}`,
            ...
            <Breadcrumbs
                ...
                {/* <hr></hr> */}
                {
                    ((ChartState) && (GETApiData.CompanyLabels.length) > 0) ?
                        <ChartComponent GETApiData={GETApiData} />
