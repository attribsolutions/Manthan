...
const Breadcrumb4 = props => {
  ...
  const { bredcrumbName, RoleAccessData_redux } = useSelector((state) => ({
    bredcrumbName: state.BreadcrumbReducer.bredcrumbName,
    RoleAccessData_redux: state.Login.RoleData,
  }));
  ...
  const NewButtonHandeller = () => {
    ...
    debugger
    RoleAccessData_redux.map((i) => {
      i.ModuleData.map((index) => {
        SearchRoleData_initial.push(index)
      })
    })
    let ralatedpageID = userPageAccess.label.RelatedPageID
    // ralatedpageID=7
    let found = SearchRoleData_initial.find((findx) => {
      return findx.id === ralatedpageID
    })
